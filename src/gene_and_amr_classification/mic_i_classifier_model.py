...
# Define df_MIC_I by dropping 'MIC' column
df_MIC_I = df.drop(columns=['MIC'])

# Drop all rows where 'MIC_Interpretation' is NaN
df_MIC_I = df_MIC_I.dropna(subset=['MIC_Interpretation'])

# Define the target variable (y) and the feature set (X)
y = df_MIC_I['MIC_Interpretation']
X = df_MIC_I.drop(columns=['MIC_Interpretation'])
...
with open(local_data_path / 'mic_i_classification_best.pkl', 'wb') as file:
    pickle.dump(model_and_encoder, file)

print("Best model and label encoder saved as 'mic_i_classification_best.pkl'")
...
with open(local_data_path / 'mic_i_classification_metrics.json', 'w') as file:
    json.dump(metrics, file)

print("Model metrics saved as 'mic_i_classification_metrics.json'")

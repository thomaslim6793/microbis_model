...
df = pd.read_csv(local_data_path / 'amr_data_long_format_no_antibiotics_preprocessed_08_24.csv',
                 dtype={'detected_variant': 'str', 'detected_binary': 'str'})

# Define df_gene_bin 
df_gene_bin = df.drop(columns=['Isolate Id', 'detected_variant'])
df_gene_bin = df_gene_bin.dropna(subset=['detected_binary'])

# Define the target variable (y) and the feature set (X)
y = df_gene_bin['detected_binary']
X = df_gene_bin.drop(columns=['detected_binary'])
...
with open(local_data_path / 'gene_bin_classification_best.pkl', 'wb') as file:
    pickle.dump(model_and_encoder, file)

print("Best model and label encoder saved as 'gene_bin_classification_best.pkl'")
...
with open(local_data_path / 'gene_bin_classification_best.json', 'w') as file:
    json.dump(metrics, file)

print("Model metrics saved as 'gene_bin_classification_best.json'")
